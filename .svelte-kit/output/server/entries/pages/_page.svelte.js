  ...
  do {
    ...
    $$rendered = `${showWrapper ? `<fieldset${add_attribute("name", params.path.join("."), 0)} class="${"subset array depth-" + escape(params.path.length, true) + " flex flex-col gap-[5px]"}">${params.collapsible || legendText ? `<legend class="subset-label array-label">${params.collapsible ? `<span class="${"collapser " + escape(collapserOpenState, true)}"></span>` : ``} <span class="subset-label-title object-label-title"><!-- HTML_TAG_START -->${stringToHtml(legendText)}<!-- HTML_TAG_END --></span> ${schema.description ? `<span class="subset-label-description object-label-description"><!-- HTML_TAG_START -->${stringToHtml(schema.description)}<!-- HTML_TAG_END --></span>` : ``}</legend>` : ``} ${collapserOpenState === "open" ? `${!emptyText ? `${each(value || [], (item, idx) => {
      return `${validate_component(SubSchemaForm || missing_component, "svelte:component").$$render(
        ...
        {}
      )} <div class="list-controls">${controls.includes("delete") ? `<button type="button" class="list-control delete" title="delete" data-svelte-h="svelte-n1bbzu">f</button>` : ``} ${controls.includes("duplicate") ? `<button type="button" class="list-control duplicate" title="duplicate" data-svelte-h="svelte-stqdoq">f</button>` : ``} ${controls.includes("reorder") && idx > 0 ? `<button type="button" class="list-control up" title="move up" data-svelte-h="svelte-1oaf89r">f</button>` : ``} ${controls.includes("reorder") && idx < (value || []).length - 1 ? `<button type="button" class="list-control down" title="move down" data-svelte-h="svelte-1yxcy74">f</button>` : ``} </div>`;
    })}` : `<div class="emptyText">${escape(emptyText)}</div>`} ${controls.includes("add") ? `<button type="button" class="list-control add" title="add item" data-svelte-h="svelte-bjrx6h">f</button>` : ``}` : ``}</fieldset>` : ``}`;
  } while (!$$settled);
  return $$rendered;
});
