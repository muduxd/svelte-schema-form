...
const Color = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${validate_component(params.components["fieldWrapper"] || missing_component, "svelte:component").$$render($$result, { params, schema }, {}, {
    default: () => {
      return `<input${add_attribute("id", params.path.join("."), 0)}${add_attribute("name", params.path.join("."), 0)} type="color"${add_attribute("value", "#ff0000", 0)} class="input" ${schema.readOnly || params.containerReadOnly ? "disabled" : ""}>`;
    }
  })}`;
});
