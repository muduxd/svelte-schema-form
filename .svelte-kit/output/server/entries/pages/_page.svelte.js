  ...
  do {
    ...
    $$rendered = `${showWrapper ? `<fieldset${add_attribute("name", params.path.join("."), 0)} class="${"subset array depth-" + escape(params.path.length, true) + " flex flex-col gap-[5px]"}">${params.collapsible || legendText ? `<legend class="subset-label array-label">${params.collapsible ? `<span class="${"collapser " + escape(collapserOpenState, true)}"></span>` : ``} <span class="subset-label-title object-label-title"><!-- HTML_TAG_START -->${stringToHtml(legendText)}<!-- HTML_TAG_END --></span> ${schema.description ? `<span class="subset-label-description object-label-description"><!-- HTML_TAG_START -->${stringToHtml(schema.description)}<!-- HTML_TAG_END --></span>` : ``}</legend>` : ``} ${collapserOpenState === "open" ? `${!emptyText ? `${each(value || [], (item, idx) => {
      return `${validate_component(SubSchemaForm || missing_component, "svelte:component").$$render(
        ...
        {}
      )} <div class="list-controls">${controls.includes("delete") ? `<button type="button" class="list-control delete btn-icon w-[35px] variant-filled-primary !mx-2" title="delete" data-svelte-h="svelte-18gvyqw">f</button>` : ``} ${controls.includes("duplicate") ? `<button type="button" class="list-control duplicate btn-icon w-[35px] variant-filled-primary !mx-2" title="duplicate" data-svelte-h="svelte-1ipf80">f</button>` : ``} ${controls.includes("reorder") && idx > 0 ? `<button type="button" class="list-control up btn-icon w-[35px] variant-filled-primary !mx-2" title="move up" data-svelte-h="svelte-9db957">f</button>` : ``} ${controls.includes("reorder") && idx < (value || []).length - 1 ? `<button type="button" class="list-control down btn-icon w-[35px] variant-filled-primary !mx-2" title="move down" data-svelte-h="svelte-1n2dhes">f</button>` : ``} </div>`;
    })}` : `<div class="emptyText">${escape(emptyText)}</div>`} ${controls.includes("add") ? `<button type="button" class="list-control add btn-icon w-[35px] variant-filled-primary !mx-2" title="add item" data-svelte-h="svelte-14d21xx">f</button>` : ``}` : ``}</fieldset>` : ``}`;
  } while (!$$settled);
  return $$rendered;
});
