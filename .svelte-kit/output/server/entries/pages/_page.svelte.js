...
const Object_1 = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    ...
    $$rendered = `<fieldset${add_attribute("name", params.path.join("."), 0)} class="${"subset object depth-" + escape(params.path.length, true) + " flex flex-col gap-[5px] " + escape(legendClasses, true) + " svelte-zseo9y"}">${showLegend ? `<legend class="subset-label object-label">${params.collapsible ? `<span class="${"collapser " + escape(collapserOpenState, true) + " svelte-zseo9y"}"></span>` : ``} ${params.containerParent !== "array" || schema.title ? `<span class="subset-label-title object-label-title !mx-2 text-lg font-bold"><!-- HTML_TAG_START -->${stringToHtml(schemaLabel(schema, params.path))}<!-- HTML_TAG_END --></span> ${schema.description ? `<span class="subset-label-description object-label-description mx-2 text-lg font-bold"><!-- HTML_TAG_START -->${stringToHtml(schema.description)}<!-- HTML_TAG_END --></span>` : ``}` : ``}</legend>` : ``} ${collapserOpenState === "open" ? `${each(propNames, (propName) => {
  ...
  do {
    ...
    $$rendered = `${showWrapper ? `<fieldset${add_attribute("name", params.path.join("."), 0)} class="${"subset array depth-" + escape(params.path.length, true) + " flex flex-col gap-[5px]"}">${params.collapsible || legendText ? `<legend class="subset-label array-label">${params.collapsible ? `<span class="${"collapser " + escape(collapserOpenState, true)}"></span>` : ``} <span class="subset-label-title object-label-title"><!-- HTML_TAG_START -->${stringToHtml(legendText)}<!-- HTML_TAG_END --></span> ${schema.description ? `<span class="subset-label-description object-label-description"><!-- HTML_TAG_START -->${stringToHtml(schema.description)}<!-- HTML_TAG_END --></span>` : ``}</legend>` : ``} ${collapserOpenState === "open" ? `${!emptyText ? `${each(value || [], (item, idx) => {
      return `${validate_component(SubSchemaForm || missing_component, "svelte:component").$$render(
        ...
        {}
      )} <div class="list-controls">${controls.includes("delete") ? `<button type="button" class="list-control delete !mx-2 bg-primary-500 p-2 rounded-full" title="delete" data-svelte-h="svelte-1cig7wg">Delete</button>` : ``} ${controls.includes("duplicate") ? `<button type="button" class="list-control duplicate !mx-2 bg-primary-500 p-2 rounded-full" title="duplicate" data-svelte-h="svelte-2rt1yk">Duplicate</button>` : ``} ${controls.includes("reorder") && idx > 0 ? `<button type="button" class="list-control up !mx-2 bg-primary-500 p-2 rounded-full" title="move up" data-svelte-h="svelte-i2n14h">Up</button>` : ``} ${controls.includes("reorder") && idx < (value || []).length - 1 ? `<button type="button" class="list-control down !mx-2 bg-primary-500 p-2 rounded-full" title="move down" data-svelte-h="svelte-woz9fd">Down</button>` : ``} </div>`;
    })}` : `<div class="emptyText">${escape(emptyText)}</div>`} ${controls.includes("add") ? `<button type="button" class="list-control add !mx-2 bg-primary-500 p-2 rounded-full" title="add item" data-svelte-h="svelte-1yb0t5l">Add</button>` : ``}` : ``}</fieldset>` : ``}`;
  } while (!$$settled);
  return $$rendered;
});
...
const ListDetail = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    ...
    $$rendered = `${showWrapper ? `<fieldset${add_attribute("name", params.path.join("."), 0)} class="${"subset array list-detail depth-" + escape(params.path.length, true) + " flex flex-col gap-[5px]"}">${params.collapsible || legendText ? `<legend class="subset-label array-label">${params.collapsible ? `<span class="${"collapser " + escape(collapserOpenState, true)}"></span>` : ``} <span class="subset-label-title object-label-title"><!-- HTML_TAG_START -->${stringToHtml(legendText)}<!-- HTML_TAG_END --></span> ${schema.description ? `<span class="subset-label-description object-label-description"><!-- HTML_TAG_START -->${stringToHtml(schema.description)}<!-- HTML_TAG_END --></span>` : ``}</legend>` : ``} ${collapserOpenState === "open" ? `${!emptyText ? `<div class="table-container" tabindex="0"${add_styles({
      ...
      })}</div> ${!readOnly ? `<div class="array-buttons"><div class="row-buttons">${controls.includes("delete") ? `<button type="button" class="list-control delete !mx-2 bg-primary-500 p-2 rounded-full" title="delete" data-svelte-h="svelte-1xfsw2z">Delete</button>` : ``} ${controls.includes("duplicate") ? `<button type="button" class="list-control duplicate !mx-2 bg-primary-500 p-2 rounded-full" title="duplicate" data-svelte-h="svelte-4ziecx">Duplicate</button>` : ``} ${controls.includes("reorder") && sort === null && idx > 0 ? `<button type="button" class="list-control up !mx-2 bg-primary-500 p-2 rounded-full" title="move up" data-svelte-h="svelte-13rp3bw">Up</button>` : ``} ${controls.includes("reorder") && sort === null && idx < (value || []).length - 1 ? `<button type="button" class="list-control down !mx-2 bg-primary-500 p-2 rounded-full" title="move down" data-svelte-h="svelte-19ateda">Down</button>` : ``}</div> </div>` : ``}`;
    })}`}</div>` : `<div class="emptyText">${escape(emptyText)}</div>`} ${controls.includes("add") ? `<button type="button" class="list-control add !mx-2 bg-primary-500 p-2 rounded-full" title="add item" data-svelte-h="svelte-1yb0t5l">Add</button>` : ``}` : ``}</fieldset>` : ``}`;
  } while (!$$settled);
  return $$rendered;
});
let type = "text";
